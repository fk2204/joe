...
class AIDisclosureTracker:
    ...
    def track_script_generation(self, video_id: str, ai_provider: str, model: Optional[str] = None):
        """
    ...
    def track_audio_generation(
        self, video_id: str, method: str, audio_type: str = "background_music"
    ):
        """
    ...
    def get_disclosure_metadata(self, video_id: str) -> DisclosureMetadata:
        ...
        max_level = max(
            (r.disclosure_level for r in records), key=lambda x: list(DisclosureLevel).index(x)
        )

        # Check if disclosure is required
        requires_disclosure = max_level in [DisclosureLevel.REQUIRED, DisclosureLevel.RECOMMENDED]
        ...
        for ct, type_records in content_type_groups.items():
            breakdown[ct.value] = {
                ...
                "disclosure_level": self.DISCLOSURE_RULES.get(ct, {})
                .get("level", DisclosureLevel.OPTIONAL)
                .value,
            }
    ...
    def _get_most_common_methods(self, rows: List, limit: int = 5) -> List[Dict]:
        ...
        return [{"method": method, "count": count} for method, count in sorted_methods[:limit]]


# Convenience functions
def create_disclosure_tracker(
    db_path: str = "data/compliance/ai_disclosure.db",
) -> AIDisclosureTracker:
